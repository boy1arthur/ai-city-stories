...
import { AiMarketingKPI } from '@/components/sponsor/AiMarketingKPI';
import { SyntheticFocusGroup } from '@/components/sponsor/SyntheticFocusGroup';
import { NativePPLEngine } from '@/components/sponsor/NativePPLEngine';
import { AffiliateTracker, createAffiliateClick, type AffiliateClick } from '@/components/sponsor/AffiliateTracker';
...
export const SponsorDashboard: React.FC<Props> = ({
  ...
  // Phase 6: Affiliate click tracking
  const [affiliateClicks, setAffiliateClicks] = useState<AffiliateClick[]>([]);

  const handleAffiliateClick = (brand: string, agentName: string) => {
    setAffiliateClicks(prev => [createAffiliateClick(brand, agentName), ...prev].slice(0, 100));
    // In production: send to analytics endpoint
    console.info(`[Affiliate] ${agentName} → ${brand}`);
  };
    ...
    <>
      <div className="min-h-screen bg-background p-6">
        <div className="max-w-6xl mx-auto">
          ...
          {/* Phase 5-3: Synthetic Focus Group */}
          <div className="mb-8">
            <SyntheticFocusGroup allAgents={allAgents} />
          </div>

          {/* Phase 6-1: Native PPL Engine */}
          <div className="mb-8">
            <NativePPLEngine
              agents={agents}
              adSlots={adSlots}
          ...
          {/* Phase 6-2: Affiliate Link Tracker */}
          <div className="mb-8">
            <AffiliateTracker clicks={affiliateClicks} />
          </div>
