...
export function selectDialogue(ctx: DialogueMatchContext): DialogueSelection | null {
  ...
  const scored = DIALOGUE_TEMPLATES.map(t => {
    ...
    // Category match
    if (t.categories !== 'all') {
      const catMatch = ctx.agent1Categories.some(c => (t.categories as string[]).includes(c));
      if (catMatch) score += 3;
      else score -= 2;
    }
  ...
  const brand = hasBrands ? ctx.nearbyBrands[Math.floor(Math.random() * ctx.nearbyBrands.length)] : '';
  ...
  return {
    ...
    emoji1: t.emoji1,
    emoji2: t.emoji2,
    brandMentioned: t.hasBrandMention ? brand : null,
